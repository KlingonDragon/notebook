# Star Trek Attack Wing
For our work 'Board-game club', we play a slimmed down version of Star Trek Attack wing.
Initially based on the "Quick Start Rules", our version introduces some rules from the full version along with some alternative additions.
Essentially, we removed actions and upgrade cards from the game.

## Set-up
- We only play with the 'generic' versions of ships.
- We try to play a game with roughly equal teams of two players controlling a single ship each.
- In cases where the number of players requires it, we may give one person multiple ships or introduce more powerful ships to balence the other teams.
- We try to use a 3'x3' play area but may increase the size when the ship count requires it.

### Obstacles
#### Planets
- Ships may not finish their manoeuvre on the planet.
- Ships may move through a planet if done in a single manoeuvre.
- Ships may not fire through a planet.

#### Other
- If a ship moves through a non-planet obstacle, its owner rolls an attack die and the ship takes damage based on the result.
- If a ship attacks through a non-planet obstacle, the target rolls an additional defence die.

## Gameplay
We play a simple two phase version of the game manoeuvre and attack.

### Manoeuvre
- Everyone selects a manoeuvre then teams take turns to move in faction order.
- We play with "Power Strain":
    - Red manoeuvres give the ship an "Auxiliary Power Token".
    - Green manoeuvres remove an "Auxiliary Power Token" from the ship.
    - Ships with an "Auxiliary Power Token" may not perform red manoeuvres.

### Attack
- Ships take turns to to attack in faction order.
- Ships may attack any ship within range and with any part of the base within the firing arc.
- We play with "Range Combat Bonus" dice:
    - Attacks at range 1 give the attacker an additional attack die.
    - Attacks at range 3 give the defender an additional defence die.
- Damage and evasion rolls apply as normal.
- Every uncancelled hit gives the target ship a damage card.
- Once a ship has as many (or more) cards as the sum of hull and shield stats, it is destroyed and removed from the game.

## Additional Notes
Many things follow the rules as given in the full "Rules of Play":
- Faction Initiative
- Fleeing the battlefield
- Moving through and overlapping ships

## Plans for potential additional tweaks
- Work out a nice way to use hull and shields rather than combining them as a summed 'health' stat.
- Work out a role for "Critical" hits.
- Add the "OP resource" "Manoeuvre Templates".
- Add basic ship actions
- Add captains and use their skill levels for move and attack orders.